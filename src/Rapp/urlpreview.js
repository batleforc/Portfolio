...
function UrlPreview(props) {
    if(!props.info) return(<div key={"urlpreviewkey"}></div>)
    return(
    <div className="cdx-block" style={{textAlign:"left",background:"grey"}} key={props.info.title+props.info.domaine}>
        <div className="link-tool" key={props.info.meta.url}>
            <a className="link-tool__content link-tool__content--rendered" key={props.info.meta.url+"link"} target="_blank" rel="nofollow noindex noreferrer" href={props.info.meta.url?props.info.meta.url:""}>
                <div className="link-tool__image" key={props.info.meta.image.url} style={{backgroundImage:"url(\""+props.info.meta.image.url+"\")"}}></div>
