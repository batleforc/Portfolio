    ...
    fetch(Profil.ip + "/projet", {
      ...
      .catch((res) => {
        if (localStorage.getItem("Projet")) {
          ...
          this.setState({
            test:
              "Le serveur contenant les information des projet est innaccessible veuillez verifier votre connexion: " +
              res,
          });
      });
  }
  render() {
    if (this.state.Projet.length === 0) return <div id="projets"></div>;
    return (
      <div
          ...
          <div className="projGrid">
            {this.state.Projet.length === 0 && (
              <h3 style={{ textAlign: "center" }}>
                Aucun projet n'est actuellement accessible
              </h3>
            )}
            {this.state.Projet.map((value, index) => {
              var todisplay =
                value.img === "" ? (
                  ...
                  <img src={value.img} alt="représentation du projet" />
                );
              return (
                <div
                  key={value.slug}
