...
app.post("/subscribe", (req, res) => {
  ...
  const payload = JSON.stringify({
    body:
      "Merci d'avoir accepter les notification" + subscription.mail
        ? subscription.mail
        : " Malgrer sa vous ne pouvez pas désactiver votre souscription, celle ci sera désactiver apres 1 mois par défault",
    title: "Portfolio Max",
    action: [
  ...
  Projet.collection(Config.mongo.notif)
    .find({ endpoint: subs.endpoint })
    .toArray(function (err, docs) {
      if (docs.length == 0) {
        Projet.collection(Config.mongo.notif).insertOne(
          subs,
          function (err, result) {
            console.log(err);
            console.log(result);
...
app.post("/unsubscribe", (req, res) => {
  ...
  Projet.collection(Config.mongo.notif).deleteOne(
    { mail },
    function (err, result) {
      assert.equal(err, null);
...
app.post("/notif", (req, res) => {
  if (!req.body.mdp)
    return res.status(418).json({ status: "Dont dream you are just a teapot" });
  bcrypt.compare(req.body.mdp, Config.password.hash, function (err, result) {
    ...
    if (result) {
      ...
      return res
        .status(418)
        .json(JSON.stringify({ status: "Dont dream you are just a teapot" }));
    }
  });
});
...
app.get("/projet", (req, res) => {
  Projet.collection(Config.mongo.article)
    .find({})
    .toArray(function (err, docs) {
      assert.equal(err, null);
